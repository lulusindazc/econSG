import glob, os
import multiprocessing as mp
import numpy as np
import imageio
import cv2
import argparse
from tqdm import tqdm
from preprocess_util import make_intrinsic, adjust_intrinsic

def process_one_scene(fn):
    '''process one scene.'''

    # process RGB images
    img_name = fn.split('/')[-1]
    img_id = int(img_name.split('_')[-1].split('.')[0])
    img = imageio.v3.imread(fn)
    img = cv2.resize(img, img_dim, interpolation=cv2.INTER_LINEAR)
    imageio.imwrite(os.path.join(out_dir_color, str(img_id)+'.png'), img)

    # process depth images
    depth_name = img_name.replace('rgb', 'depth')
    fn_depth = os.path.join(os.path.dirname(fn).replace('rgb','depth'), depth_name)
    depth = imageio.v3.imread(fn_depth).astype(np.uint16)
    depth = cv2.resize(depth, img_dim, interpolation=cv2.INTER_LINEAR)
    imageio.imwrite(os.path.join(out_dir_depth, str(img_id)+'.png'), depth)

    # process label images
    semantic_name = img_name.replace('rgb', 'semantic_class')
    fn_semantic = os.path.join(os.path.dirname(fn).replace('rgb','semantic_class'), semantic_name)
    semantic = imageio.v3.imread(fn_semantic)
    semantic = cv2.resize(semantic, img_dim, interpolation=cv2.INTER_LINEAR)
    imageio.imwrite(os.path.join(out_dir_label, str(img_id)+'.png'), semantic)


    #process poses
    np.savetxt(os.path.join(out_dir_pose, str(img_id)+'.txt'), pose_list[img_id])
    
def process_txt(filename):
    with open(filename) as file:
        lines = file.readlines()
        lines = [line.rstrip() for line in lines]
    return lines



if __name__ == '__main__':

   
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset_path', type=str, default='replica_2d') #required=True)
    parser.add_argument('--in_path', type=str, default='replica') #required=True)
    parser.add_argument('--scene_name', type=str, default='office_0')
    args = parser.parse_args()


    out_dir = args.dataset_path 
    in_path = args.in_path 
    scene_list = [args.scene_name] 
    #####################################

    sample_freq = 1
    os.makedirs(out_dir, exist_ok=True)

    # ####### Meta Data #######
    # scene_list = ['office0', 'office1', 'office2', 'office3',
    #             'office4', 'room0', 'room1', 'room2']
    img_dim = (640, 480) #(320, 240) #
    # original_img_dim = (1200, 680)
    # intrinsic parameters on the original image size
    intrinsics = make_intrinsic(fx=320, fy=320, mx=(640-1)*0.5, my=(480-1)*0.5)

    # save the intrinsic parameters of resized images
    # intrinsics = adjust_intrinsic(intrinsics, original_img_dim, img_dim)
    

    for scene in tqdm(scene_list):
       
        # os.makedirs(os.path.join(out_dir, scene), exist_ok=True)
        scene_wo_= scene.replace('_','')
        out_dir_color = os.path.join(out_dir, scene_wo_, 'color')
        out_dir_depth = os.path.join(out_dir, scene_wo_, 'depth')
        out_dir_pose = os.path.join(out_dir, scene_wo_, 'pose')
        out_dir_label = os.path.join(out_dir, scene_wo_, 'label')
        if not os.path.exists(out_dir_color):
            os.makedirs(out_dir_color)
        if not os.path.exists(out_dir_depth):
            os.makedirs(out_dir_depth)
        if not os.path.exists(out_dir_pose):
            os.makedirs(out_dir_pose)
        if not os.path.exists(out_dir_label):
            os.makedirs(out_dir_label)

        np.savetxt(os.path.join(out_dir, scene_wo_ , 'intrinsics.txt'), intrinsics)
        # save the camera parameters to the folder
        camera_dir = os.path.join(in_path,
                scene, 'traj_w_c.txt')
        poses = np.loadtxt(camera_dir, delimiter=" ").reshape(-1, 4, 4)
        pose_list = poses[::sample_freq]

        files = glob.glob(os.path.join(in_path, scene, 'rgb', '*.png'))
        files = sorted(files)
        files = files[::sample_freq] # every 10 frames
      
        p = mp.Pool(processes=mp.cpu_count())
        p.map(process_one_scene, files)
        p.close()