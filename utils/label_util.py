...
def get_palette(num_cls=21, colormap='scannet'):
    ...
    elif colormap == 'replica':
        # scannet_palette = []
        # # import pdb
        # # pdb.set_trace()
        # for value in label_colormap():#REPLICA_COLOR_MAP_24.items():
        #     scannet_palette.append(np.array(value))
        # palette = np.concatenate(scannet_palette)
        total_num_classes=101
        palette = np.concatenate(label_colormap(total_num_classes)[np.array(REPLICA_EXISTING_CLASSES).astype(np.uint8)])
