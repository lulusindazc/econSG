...
from utils.graphics_utils import BasicPointCloud,SemanticPointCloud
...
class GaussianModel:
    ...
    def restore(self, model_args, training_args, mode='train'):
        ...
        self.denom = denom
        if not training_args.train_feature_only: 
    ...
    def get_language_feature(self):
        if self._language_feature is not None:
            ...
            raise ValueError('Undefined language feature')
    ...
    def create_from_pcd(self, pcd : SemanticPointCloud, spatial_lr_scale : float):
        ...
        # tmp=distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda())
        # import pdb
        # pdb.set_trace()
        ...
        language_feature = torch.tensor(np.asarray(pcd.semantics)).float().cuda() #torch.zeros((fused_point_cloud.shape[0], 512), device="cuda")
    ...
    def training_setup(self, training_args):
        ...
        if not training_args.train_feature_only:
            print('CHECK:===============Training setup with ALL parameters=============')
            l = [
            ...
            print('CHECK:===============Training setup with ONLY FEATURE parameters=============')
            l = [
                {'params': [self._language_feature], 'lr': training_args.language_feature_lr, "name": "language_feature"}, # TODO: training_args.language_feature_lr
            ]
    ...
    def finetune_setup(self, training_args, mask3d):
        ...
        self._language_feature.requires_grad = False
        ...
        l = [
            ...
            {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
            {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"},
            {'params': [self._language_feature], 'lr': training_args.feature_lr, "name": "language_feature"},
        ]
    ...
    def removal_setup(self, training_args, mask3d):
        ...
        language_feature_sub = self._language_feature[mask3d].detach()
        ...
        self._language_feature = nn.Parameter(set_requires_grad(language_feature_sub, False))
    ...
    def inpaint_setup(self, training_args, mask3d):
        ...
        def initialize_new_features(features, num_new_points, mask_xyz_values, distance_threshold=0.25, max_distance_threshold=1, k=5):
            ...
            return new_features['xyz'], new_features['features_dc'], new_features['scaling'], new_features['language_feature'], new_features['features_rest'], new_features['opacity'], new_features['rotation']
        ...
        sub_features = {
            ...
            'scaling': scaling_sub,
            'language_feature': language_feature_sub,
        ...
        with torch.no_grad():
            new_xyz, new_features_dc, new_scaling, new_language_feature, new_features_rest, new_opacity, new_rotation = initialize_new_features(sub_features, num_new_points, mask_xyz_values)
        ...
        self._language_feature = nn.Parameter(torch.cat([set_requires_grad(language_feature_sub, False), set_requires_grad(new_language_feature, True)]))
        ...
        l = [
            ...
            {'params': [self._scaling], 'lr': training_args.scaling_lr, "name": "scaling"},
            {'params': [self._rotation], 'lr': training_args.rotation_lr, "name": "rotation"},
            {'params': [self._language_feature], 'lr': training_args.language_feature_lr, "name": "language_feature"}  # Assuming there's a learning rate for language_feature in training_args
        ]
    ...
    def save_ply(self, path):
        ...
        semantics = self._language_feature.detach().cpu().numpy()

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes()]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, semantics, opacities, scale, rotation), axis=1)
    ...
    def load_ply(self, path):
        ...
        semantics_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("sem_")]
        ...
        for idx, attr_name in enumerate(semantics_names):
            # semantics_list.append(np.asarray(plydata.elements[0][attr_name]))
            semantics[:, idx] = np.asarray(plydata.elements[0][attr_name])
         #np.stack(semantics_list,  axis=1)
        # semantics = np.stack((np.asarray(plydata.elements[0]["sem_0"]),
        #                 np.asarray(plydata.elements[0]["sem_1"]),
        #                 np.asarray(plydata.elements[0]["sem_2"])),  axis=1)
        ...
        self._language_feature = nn.Parameter(torch.tensor(semantics, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_language_feature,new_opacities, new_scaling, new_rotation):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_language_feature, new_opacity, new_scaling, new_rotation)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_language_feature, new_opacities, new_scaling, new_rotation)
